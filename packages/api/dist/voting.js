...
function impact(voteType, weight) {
    return voteType === 'UP' ? weight : -weight;
}
async function targetExists(prisma, targetId, targetType) {
...
async function applyScoreDelta(prisma, targetId, targetType, scoreDelta) {
    if (scoreDelta === 0)
        return;
    if (targetType === 'QUESTION') {
...
export async function castVote({ prisma, userId, targetId, targetType, voteType }) {
