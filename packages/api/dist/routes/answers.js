...
function calculateQualityScore(body, voteScore, isExpertVerified, isAccepted) {
...
const answerRoutes = async (fastify) => {
    ...
    fastify.post('/', {
        ...
        const answer = await fastify.prisma.answer.create({
            data: {
                ...
                status: initialStatus, // Cast to AnswerStatus
            },
        });
    ...
    fastify.delete('/:id', async (req, reply) => {
        ...
        if (!authUser)
            return;
        if (existing.authorId !== authUser.sub)
    ...
    fastify.post('/:id/vote', async (req, reply) => {
        ...
        if (value !== 1 && value !== -1)
            return reply.status(400).send({ error: 'Invalid vote value' });
        const authUser = await requireAuth(req, reply);
        if (!authUser)
            return;
