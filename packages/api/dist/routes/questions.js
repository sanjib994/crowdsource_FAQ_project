...
const questionRoutes = async (fastify) => {
    ...
    fastify.post('/', {
        ...
        if (keywords.length > 0) {
            ...
            const matches = potentialDuplicates.filter((q) => {
                const qKeywords = q.title.toLowerCase().split(/\W+/);
                const matchCount = keywords.filter(kw => qKeywords.some((qw) => qw.includes(kw) || kw.includes(qw))).length;
    ...
    fastify.get('/:id', async (req, reply) => {
        ...
        if (question.answers && question.answers.length > 0) {
            ...
            const scoredAnswers = question.answers.map((a) => {
                let score = a.voteScore * 10;
                if (a.isExpertVerified)
                    score += 500;
                if (a.isAccepted)
                    score += 1000;
            ...
            scoredAnswers.sort((a, b) => b._compositeScore - a._compositeScore);
    ...
    fastify.post('/:id/answers', async (req, reply) => {
        ...
        if (!body || body.length < 10)
            return reply.status(400).send({ error: 'Body too short' });
